...
`-.`.____.'       `--`--''     `--`-`--`--'   `--`           
    \n""")
    try:
    	ny1,ny2,ny3,save = input(f"{lrd}[{lgn}+{lrd}] {lgn}Dork {k}: {cn}"),int(input(f"{lrd}[{lgn}+{lrd}] {lgn}Number Page{k} : {cn}")),int(input(f"{lrd}[{lgn}+{lrd}] {lgn}Timeout {k}: {cn}")),input(f"{lrd}[{lgn}+{lrd}] {lgn}save location : {k}")
    	nyx = 0
    	for i in search(ny1, tld="com", lang="en", num=int(ny2), start=0, stop=None, pause=int(ny3)):
    		with open(save, 'a') as f:
    			f.write(f'{i}\n')
    		nyx += 1
    		print(f'{lrd}[{lgn}{nyx}{lrd}] {yw}=>> [{k}{i}{lrd}]')
    		if nyx >= int(ny2):
    			break;
    	print(f'\n{lrd}[{lgn}+{lrd}] {gn}Saved : {lgn}results.txt')
    except ValueError:
...
def dork_List():
 ...
 L:                                               
    \n""")
    try:
    	ny1,ny2,ny3,save = input(f"{lrd}[{lgn}+{lrd}] {lgn}Dork File List{k}: {cn}"),int(input(f"{lrd}[{lgn}+{lrd}] {lgn}Number Page{k} : {cn}")),int(input(f"{lrd}[{lgn}+{lrd}] {lgn}Timeout {k}: {cn}")),input(f"{lrd}[{lgn}+{lrd}] {lgn}save location : {k}")
    	f = open(ny1,'r')
    	nyx = 0
    	for file in f:
    	    for i in search(file, tld="com", lang="en", num=ny2, start=0, stop=None, pause=int(ny3)):
    		    with open(save, 'a') as f:
    		    	f.write(f'{i}\n')
    		    	nyx += 1
    		    	print(f'{lrd}[{lgn}{nyx}{lrd}] {yw}=>> [{k}{i}{lrd}]')
...
| '--------------' || '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'
	...
	print ("{lrd}Enter the appropriate option ")